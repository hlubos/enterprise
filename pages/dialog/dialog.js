...
Page({
  ...
  onLoad: function (option) {
    ...
    switch (option.dialogType) {
      case 'isRemind':
        ...
        if (!temp) return
        if (
          JSON.parse(temp).some(
            (el) => el.enterprise_id == this.data.enterprise_id,
          )
        ) {
          this.setData({
  ...
  onShareAppMessage() {
    return {
      ...
      path:
        '/pages/login/login?to=invite&enterprise_id=' +
  ...
  async remindStaff() {
    ...
    if (!remindedEnterprises) {
      ...
      remindedEnterprises = remindedEnterprises.filter(
        (el) => el.enterprise_id != data.enterprise_id,
      )
      remindedEnterprises.push(data)
      wx.setStorageSync('isAlreadyRemind', JSON.stringify(remindedEnterprises))
    }
  ...
  upload() {
    ...
    uploadFile.upload({
      ...
      success(obj) {
        ...
        wx.navigateTo({
          url:
            '/pages/index/index?to=createCompany' + '&imgUrl=' + obj.orig_url,
        })
      },
    })
  },
