...
Page({
  ...
  onLoad: function (option) {
    ...
    if ('loginAagin' in option) {
      let { enterprise_id, invitor_user_id } = option
  ...
  async jumpTo() {
    let user_id = (await tool.getYdStorage('user_id')) || 0
    let xyy = (await tool.getYdStorage('xyy')) || 'zache'
    if (this.enterprise_id && this.invitor_user_id) {
      //要跳回邀请加入页的情况
      wx.navigateTo({
        url:
          '/pages/index/index?to=login&enterprise_id=' +
          this.enterprise_id +
          '&invitor_user_id=' +
          this.invitor_user_id +
          '&user_id=' +
      ...
      wx.navigateTo({
        url:
          '/pages/index/index?user_id=' + user_id + '&xyy=' + xyy + '&to=login',
      })
    }
  },
  login() {
