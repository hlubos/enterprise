...
Page({
  ...
  data: {
    cameraBlockHeight:
      app.globalData.systemInfo.screenHeight - app.globalData.CustomBar,
    tipsText: '请站在识别框内',
    predicting: false,
    videoWidth: null,
    videoHeight: null,
   ...
   */
  onUnload: function () {
    this.data.sportStart && !this.data.sportEnd && this.uploadScore()
    wx.setKeepScreenOn({
      keepScreenOn: false,
    })
  ...
  initClassifier() {
    ...
    var a = i.windowWidth / 2 - 0.2 * i.windowHeight,
      n = i.windowWidth - a
    this.setData({
  ...
  executeClassify(frame) {
    ...
    if (this.classifier && this.classifier.isReady() && !this.data.predicting) {
      this.setData(
        ...
        () => {
          this.classifier
            .detectSinglePose(frame, 'multiple')
            .then((pose) => {
              this.setData({
                predicting: false,
              })
              ...
              if (
                that.data.limitTime > 0 &&
                that.data.lastTime <= 0 &&
                !that.data.sportEnd
              ) {
  ...
  calculateScore: function (pose) {
    ...
    if (a.satisfyConf(m, 0.2, g) && a.isInBox(g)) {
      if (a.data.sportStart || a.countDownTimer) {
        if (!a.data.countDown) {
          a.isOutScreen()
          var p = m[11].position.y,
            f = m[12].position.y,
            v = m[5].position.y,
            w = m[6].position.y,
            D = m[15].position.y,
  ...
  satisfyFrame: function (t, e) {
    var i = this,
      a = !0,
      n = 20 * this.pixelRatio
    return (
      Array.isArray(e) &&
        Array.isArray(t) &&
        e.forEach(function (e) {
          ;(!t[e] ||
            t[e].position.x < i.left_point_pos - n ||
            t[e].position.x > i.right_point_pos + n ||
            t[e].position.y > 0.9 * i.videoHeight + n ||
            t[e].position.y < 0.15 * i.videoHeight - n) &&
            (a = !1)
        }),
      a
    )
  ...
  startSport: function () {
    ...
    that.costTimer = setInterval(function () {
      let timeProgress = that.data.limitTime
        ? that.data.costTime * (100 / that.data.limitTime)
        : 0
      that.setData({
  ...
  confirm() {
    ...
    if (param.cost_time == 0 || param.action_times == 0) {
      wx.navigateBack()
    } else {
      api.reportUserAISportData(param).then((res) => {})
      wx.navigateBack()
      ...
      if (!n || e) {
        i.notMovingTimer ||
          (i.notMovingTimer = setInterval(function () {
            i.setData(
              {
                notMovingTime: i.data.notMovingTime
                  ? i.data.notMovingTime + 1
                  : 1,
              },
              function () {
