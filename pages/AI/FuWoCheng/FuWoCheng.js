...
Page({
    ...
    satisfyFrame: function (t, i) {
        var a = this, e = !0, o = 40 * this.data.pixelRatio;
        return Array.isArray(i) && Array.isArray(t) && i.forEach(function (i) {
            (!t[i] || t[i].position.x < a.data.left_point_pos - 2 * o || t[i].position.x > a.data.right_point_pos + 2 * o || t[i].position.y > .9 * a.data.videoHeight + 40 * a.data.pixelRatio + o || t[i].position.y < .1 * a.data.videoHeight - o) && (e = !1);
        }), e;
    },
    ...
    matchPoint: function (t, i, a) {
        ...
        return t.forEach(function (t) {
            (Math.abs(t.position.x - i) > 30 * e.data.pixelRatio || Math.abs(t.position.y - a) > 30 * e.data.pixelRatio) && (o = !1);
        }), o;
    },
    ...
    isMoving: function (t) {
        ...
        return 5 === e.length && (e.forEach(function (o, n) {
            if (0 !== n) {
                var r = !1;
                t.forEach(function (t) {
                    var a = Math.abs(o[t].position.x - e[n - 1][t].position.x), s = Math.abs(o[t].position.y - e[n - 1][t].position.y);
    ...
    isOutScreen: function () {
        ...
        var e = [5, 6, 7, 8], o = a.isMoving(e);
        a.data.time != a.data.recordTime && (a.setData({
            recordTime: a.data.time
        }), (!o || i) && a.data.time > 10 ? a.notMovingTimer || (a.notMovingTimer = setInterval(function () {
            a.setData({
        ...
        this.classifier && this.classifier.isReady() && !this.data.predicting && this.setData({
            ...
            i.classifier.detectSinglePose(t).then(function (t) {
                ...
                if (!e.data.gameEnd) {
                    ...
                    var n = [5, 6], r = e.satisfyConf(o, .2, n), s = (o[15].position.y + o[16].position.y) / 2, d = (o[9].position.y + o[10].position.y) / 2, c = o[0].position.y;
                    if (r) if (e.data.gameStart || e.timer) {
                        if (!e.data.countDown) {
                            e.isOutScreen();
                            var l = Math.abs(o[7].position.y + o[8].position.y - o[5].position.y - o[6].position.y) / 2, h = (o[5].position.y + o[6].position.y) / 2;
                            if (e.data.show_tip && e.setData({
                                show_tip: !1
                            }), e.data.doUp && !1 === e.data.actionAry[e.data.actionAry.length - 1]) 0 === e.data.num && e.setData({
                                halfArmLength: l,
                                shoulder_elbow_mid_line: Math.abs(o[7].position.y + o[8].position.y + o[5].position.y + o[6].position.y) / 4
                            }), (0 === e.data.num || h < e.data.shoulder_elbow_mid_line - 30) && e.data.canAdd && (e.data.num && e.playMusic("appear"),
                                e.setData({
                                    num: e.data.num + 1,
                                    doUp: !1,
                                ...
                                }, 300)); else if (!0 === e.data.actionAry[e.data.actionAry.length - 1] && !e.data.doUp) {
                                    var u = (0, a.getJointAngle)(o[7].position, o[5].position, o[9].position), p = (0,
                                        a.getJointAngle)(o[8].position, o[6].position, o[10].position), m = u < 130 || p < 130;
                                    h > e.data.shoulder_elbow_mid_line + 20 && m && e.setData({
                                        doUp: !0,
                        ...
                        var g = !e.data.gameStart && s > .9 * e.data.videoHeight + 20 * e.data.pixelRatio, f = c > .25 * e.data.videoHeight + 20 * e.data.pixelRatio, v = (o[9].position.y + o[10].position.y - o[6].position.y - o[5].position.y) / 2, w = (o[15].position.y + o[16].position.y - o[11].position.y - o[12].position.y) / 2;
                        e.isInBox(n) && !g && f && v > w && d < .95 * e.data.videoHeight && e.setData({
                            countDown: 3,
                            countDownSrc: e.data.countDownSrcAry[2]
                        }, function () {
                            i.triggerGameStart();
                        });
                    } else e.data.gameStart && !e.isInBox(n) && e.setData({
                        show_tip: !0
    ...
    initClassifier: function () {
        ...
        this.classifier = new t.Classifier("front", {
            ...
            left_point_pos: (a.windowWidth - .35 * a.windowHeight) / 2,
            right_point_pos: a.windowWidth - (a.windowWidth - .35 * a.windowHeight) / 2,
