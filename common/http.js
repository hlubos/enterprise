...
const post = async (url, parms = {}, isfromGetsskey) => {
	...
	return new Promise((resolve, reject) => {
		...
		if (data.code !== 0 && data.code !== 4004 && data.code !== 7007 && data.msg !== 'ok') {
			if (data.code === 1 && data.msg === '参数不合法') { //将参数不合法的请求上报
				let timeSpace = 0;
				if (res.header.server_time) {
					timeSpace = parseInt(res.header.server_time) - Date.parse(new Date()) / 1000;
				}
			...
			} else if (!parms.hijack) { //参数hijack，如果存在且为true,则弹框信息业务处理
				wx.showToast({
					title: data.msg,
					icon: 'none'
				})
		...
		} else { //弱网下按钮跳转的事件上报被cancel时，不用出toast弹窗
			wx.showToast({
				title: '当前网络不给力，请稍后重试',
				icon: 'none'
			})
